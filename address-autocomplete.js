...
class AddressAutoComplete {
  ...
  enableSearchButton() {
    document.getElementById(this.buttonId).classList.remove('disabled');
  }


  disableSearchButton() {
    document.getElementById(this.buttonId).classList.add('disabled');
  }
  ...
  requestForm() {
    // Redirect til /beregner/[URL-encoded adresse]
    if (!this.addressText) {
...
setTimeout(() => {
  ...
  input.addEventListener('input', function() {
    ...
    // Kun start timer hvis der er nok tekst (mindst 10 karakterer)
    if (inputValue.length >= 10) {
      console.log('Starter auto-submit timer for:', inputValue);

      // Start timer - auto-submit efter 2 sekunder uden aktivitet
